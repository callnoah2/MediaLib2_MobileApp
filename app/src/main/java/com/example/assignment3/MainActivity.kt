...
//                                    boardGameCount = BoardGamesRepository.boardGameId,
                            ...
                            composable("movie_detail/{movieId}") { backStackEntry ->
                                val movieId = backStackEntry.arguments?.getString("movieId")?.toLongOrNull() ?: 0
                                MovieDetailScreen(
                                    movieId = movieId,
                                    navController = navController
                                )
                            ...
                            composable("book_detail/{bookId}") { backStackEntry ->
                                val bookId = backStackEntry.arguments?.getString("bookId")?.toLongOrNull() ?: 0
                                BookDetailScreen(
                                    bookId = bookId,
                                    navController = navController
                                )
                            ...
                            composable("video_game_detail/{videoGameId}") { backStackEntry ->
                                val videoGameId = backStackEntry.arguments?.getString("videoGameId")?.toLongOrNull() ?: 0
                            ...
                            composable("board_game_detail/{boardGameId}") { backStackEntry ->
                                val boardGameId = backStackEntry.arguments?.getString("boardGameId")?.toLongOrNull() ?: 0
